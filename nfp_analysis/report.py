...
def build_report(output: str | Path = "output/nfp_report.xlsx", start: str = "2022-01-01") -> Path:
    ...
    notes = [
        ...
        "Strategy: trade only when the trailing 24-release bias t-stat clears +/-1.5; use the top-5 "
        "agreement only to scale size (1.5x aligned / 0.5x opposed), never to initiate. Express via "
        "front-end rates or USD into the print. This is a sizing/skew framework for a pre-NFP view, "
        "not a money machine: ~50 tradeable events per decade, regime-dependent, first-print scored.",
    ]
